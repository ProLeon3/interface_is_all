...
go run -buildvcs=false ./cli -file /tmp/my-todos.json add -due 2026-10-01 '提交设计评审'
...
`-file` 放在子命令前；默认文件为当前目录的 `tasks.json`。`add` 可在标题前用 `-due YYYY-MM-DD` 设置截止日期（可省略），输出 ID、标题，有截止日期时再输出「截止 日期」。`list` 只列出未完成任务，按截止日期从早到晚排列，没有截止日期的排在最后，同一天按 ID 排。`done` 支持重复执行。空白标题、无效截止日期、未知或无效 ID、损坏的数据文件均返回非零退出状态。没有 `due` 字段的旧数据文件可直接使用。完成任务不复用 ID，重复完成不重写文件。
...
| `task-service` | [task/task.go](task/task.go)，`Add`、`Complete`、`Pending` | 校验业务文档、标题、截止日期与 ID；未完成列表按截止日期升序、无日期居后、同日按 ID；通过 `storage` 读写 |
...
Go 依赖检查确认未违反四条显式禁止规则，状态为 `passed`；模型语义审查仍为 `not_run`。上述对应关系由 coding agent 对照代码核查，用户仍可进一步审阅语义。业务测试覆盖进程调用式持久化、创建/完成/列表、重复完成不额外写入、无效操作不改数据、损坏文档不覆盖，以及截止日期的排序、无效日期拒绝与旧文档兼容。
