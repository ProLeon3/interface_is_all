...
> 2026-09-21 已确认新方向：由外部 coding agent 主导流程并调用本程序，所有 LLM 推理由 agent 承担；程序保留源码事实采集、确定性检查、图形编辑、提案审阅、存储与用户确认。决定见 [ADR-0002](docs/adr/0002-external-coding-agent-orchestration.md)。代码迁移尚未实施，下文仍说明当前可运行的功能，包括内置模型配置与生成入口。

本项目提供本地浏览器工作台、Go 核心库和命令行工具：保存模块职责、接口能力及禁止依赖规则，由用户审阅确认后作为检查基准，并为外部 AI 工具提供能力审查协议。当前存储与检查边界见[设计存储与验证](设计存储与验证.md)。

已按[独立图形工作台实现依据](独立图形工作台实现依据.md)实现需求输入、AI 生成方案、接口协作图、围绕所选对象提出修改、图形变更审阅、草稿接受、版本确认和手动交接。沿用项目隔离、版本冲突检测、未保存编辑保护、独立布局与原有检查能力。技术选择与验证边界见[独立工作台实现与验收](docs/standalone-workbench.md)。

已按[已有项目初次分析实现依据](已有项目初次分析实现依据.md)实现已有 Go 项目从代码开始的首次接入：分析源码、审阅附带依据的现状提案、编辑草稿、明确确认，再使用原有检查。实现选择和验证边界见[初次分析实现与验收](docs/initial-analysis.md)。

已有确认基准的项目也可以重新分析最新源码，与旧基准和已保存草稿对照，接受后再明确确认新版本。使用方式与存储保护见[已有基准项目重新分析](docs/reanalysis.md)。

skill 安装、外部对话同步、自动交接与自动修复尚未实现；设计生成与源码分析仍需接入外部 agent 提案。依赖检查通过不表示接口协作、业务功能或模型语义已经通过验收。
...
在浏览器打开终端显示的地址，默认是 `http://127.0.0.1:8090`。点击「当前项目」旁的「选择项目」，输入路径或逐级浏览目录即可切换，无需重启服务。`-project` 指定初始项目，省略时使用启动目录。每个标签页独立选择项目，刷新后恢复该标签页的选择；切换前若有未保存编辑，可以取消后保存、下载草稿备份，或明确同意丢弃。`-timeout` 约束单次扫描或模型请求，`-tags` 指定构建标签，不会让工作台按检查超时退出。
...
1. 空项目先选择起草方式。已有 Go 代码默认进入代码分析；从需求开始时选择「从需求设计」，输入需求并点击「生成设计初稿」。AI 结果进入待审阅提案，不会自动保存草稿或确认设计。
2. 已有设计默认先显示协作图。点选模块、具体接口或协作线，展开「调整设计」，在「本次修改意图」描述调整。图上显示新增、修改、迁移和删除；可以查看修改前、继续调整，或在「提案工具」中放弃提案。已有未保存编辑时先保存草稿。
3. 点击「接受并保存草稿」，再单独点击「审阅并确认」，查看包含协作关系的完整设计，填写确认人并勾选审阅声明。接受提案不等于确认整个版本。
...
选择项目根目录含 `go.mod`、尚无确认设计的项目，在代码分析入口点击「分析已有代码」。已有草稿时先展开「调整设计」，再选择代码分析方式。无需先手写设计；模型会收到本次读取的 Go 源码、公开声明候选、包依赖和构建清单。打开目录本身不扫描，也不调用模型。

在图上核对模块、能力和协作；详情中的「初次分析依据」可以查看分析时的原始源码。图下分别列出读取范围、排除文件、问题、建议与不确定项。包导入只证明依赖，未观察到某条依赖不会自动变成禁止规则。间接调用或动态关系不能静态关联时标为不确定。

点击「接受并保存草稿」后，可以沿用表单编辑模块、接口归属和协作。来源证据会保留，后续编辑不会改写原分析。最后单独「审阅并确认」，建立第一个基准，再到「代码检查」运行检查。待审阅分析保存在项目中，刷新或另一窗口重新打开都可恢复。

首次分析沿用单 Go module 的扫描范围。源码加载不完整、没有可读 Go 文件或完整上下文超过 **1 MiB** 时停止生成，页面展示范围和原因；不静默截断源码或保存部分分析草稿。模型自身的上下文上限可能更小，模型报错时原草稿仍保留。

生成完成、接受及确认本次分析时会重新核对源码指纹和构建范围，草稿与基准同时受版本检查保护。源码过期时可明确撤回并重新分析；已接受过的来源撤回后仍保留保护标记，必须接受新提案后才能确认。

### 已有基准项目重新分析

在「设计草稿」视图展开「调整设计」，选择代码分析方式，再点击「重新分析代码」。原草稿和旧基准继续保留，生成的提案分别展示相对已保存草稿、相对已确认基准的变更。现状分析不自动保留旧禁止规则，差异会明确列出；接受后可编辑草稿恢复需要的约束。

「接受并保存草稿」只更新草稿；最后「审阅并确认」才发布新基准。确认前的检查和设计交接仍使用旧基准，历史快照和分析证据保留。即使新分析没有改变设计文字，也可以确认新的源码依据。刷新或重开工作台能恢复当前分析。

### 模型配置

将 [.env.archdesign.example](.env.archdesign.example) 复制为 `.env.archdesign` 并填写环境变量，建议执行 `chmod 600 .env.archdesign`。本地文件已加入忽略规则，不会由工作台接口返回。

| 环境变量 | 说明 |
| --- | --- |
| `ARCHDESIGN_AI_BASE_URL` | API 基础地址，例如 `https://服务地址/v1`，不含 `/responses` 或 `/chat/completions` |
| `ARCHDESIGN_AI_MODEL` | 服务支持的模型名 |
| `ARCHDESIGN_AI_API_KEY` | 密钥；本机无鉴权服务可留空 |
| `ARCHDESIGN_AI_API` | `chat_completions`（默认）或 `responses` |
| `ARCHDESIGN_AI_REASONING_EFFORT` | 可选推理强度，例如 `xhigh`；须由所选模型支持 |
| `ARCHDESIGN_AI_RESPONSE_FORMAT` | `json_schema`（默认，严格输出）或兼容旧服务的 `json_object` |

```sh
# 自动加载本地环境文件并构建；服务只监听本机。
bash scripts/start-workbench.sh -project /你的项目目录 -timeout 5m

# 自定义配置路径。
ARCHDESIGN_ENV_FILE=/你的配置路径 bash scripts/start-workbench.sh
```

也可自行设置环境变量后运行 `archdesign serve`。未配置模型时仍能使用手动编辑与检查。普通需求设计接收需求、修改意图、选择对象和当前设计；首次、重新分析及其提案调整还接收实际读取范围内的源码和构建清单。服务不向模型发送会话令牌或模型凭据作为上下文。配置错误、超时、截断、拒绝或无效设计会显示错误并保留原草稿，不自动重试或更换模型。
...
.architecture/
  ...
  proposals/<id>.json             # 不可变 AI 提案，源码分析保存源码及依据，重新分析另存原基准
...
也可提供可执行审查程序，自动完成提取、调用和结果导入：

```sh
/tmp/archdesign review -project examples/shop \
  -reviewer /绝对路径/审查程序 -timeout 5m
```

程序从标准输入读取一个请求 JSON，将一个响应 JSON 写到标准输出；日志写到标准错误。需要参数或模型凭据时由该程序的包装脚本配置。工具直接执行此路径，不通过 shell 解析命令字符串；能力审查的供应商仍由外部程序配置；工作台设计生成使用上面的环境变量，两条流程互不代替。
...
所有语义关联和问题都标记 `pending_confirmation`，原始设计依据由程序从确认快照附加。`aligned` 仍是模型判断，`not_found` 表示“在本次范围尚未找到”，不是确定性能力缺失。`review` 输出中的 `check` 是调用模型前的确定性报告，其 `semantic_status` 保留 `not_run`；本次语义结果单独放在 `semantic` 中。

当前校验能够保证结构、引用与证据位置可核对，不能证明模型理由正确或没有漏报。仓库测试使用固定响应和本地替身程序验证接入流程，没有使用真实模型验证判断准确率。
...
| `designer` | OpenAI 兼容模型调用、输出协议与错误处理 |
...
go test -race ./...
go vet ./...
# 以下需要 playwright-cli 和可用的 Chromium。
bash scripts/verify-workbench.sh
bash scripts/verify-ai-workbench.sh
```

测试覆盖草稿与基准隔离、陈旧版本拒绝、并发确认、历史完整性、设计结构和关系校验、符号链接边界、子包入口、允许与禁止的直接依赖、改名与参数重组、构建标签、扫描失败、缺失能力和新增能力响应、无效证据、过期审查、外部程序取消，以及命令行完整流程。它们验证本地程序行为，不代表真实模型效果或外部工具自动触发已经验证。浏览器流程的契约与执行命令见[核心用户旅程](docs/core-user-journeys.md)。
